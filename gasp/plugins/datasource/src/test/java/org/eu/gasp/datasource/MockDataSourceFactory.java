...
public class MockDataSourceFactory extends AbstractDataSourceFactory {
    @Override
    protected DataSource doCreate(DataSourceDescriptor desc) throws Exception {
        final MockControl control = MockControl.createControl(DataSource.class);
        final DataSource ds = (DataSource) control.getMock();
        control.replay();
