...
public class DefaultPluginRegistryTest extends TestCase {
    private final Log log = LogFactory.getLog(getClass());


    public void testInstall() throws Exception {
        final String urlPath = "/testplugin-1.0.jar";
        final URL url = getClass().getResource(urlPath);
        assertNotNull("URL introuvable: " + urlPath, url);

        log.info("URL: " + url);

        final PluginRegistry pluginRegistry = new DefaultPluginRegistry();
        pluginRegistry.install(url);
    }
}
